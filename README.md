...
当前版本：**0.12.1（22）**。支持 iPhone / iPad（iOS 17+）和 Mac（macOS 14+，Mac Catalyst）。
...
月历默认显示农历：初一显示月份、闰月标明「闰」，春节、元宵、端午、七夕、中元、中秋、重阳、腊八、龙抬头和除夕显示节日名。当日日程与日程列表显示完整农历日期。使用系统中国历离线换算，按当前页面的公历日期对应中国标准时间的农历，不创建额外日程或节日提醒；闰月不重复标注节日。暂不包含节气、调休或农历重复事件规则。
